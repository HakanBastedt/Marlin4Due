...
void hakans_fsr_setvalue(int pulses)
{
  pinMode(HAKANS_FSR_PIN, OUTPUT);
  for (int i=0; i < pulses; i++) {
    delay(1); // 1 milliseconds, gÃ¥r sÃ¤kert fortare Ã¤n sÃ¥.
    digitalWrite(HAKANS_FSR_PIN, HIGH);   
    delay(1); 
    digitalWrite(HAKANS_FSR_PIN, LOW);    
  }    
  delay(100); 
}

void hakans_fsr_setzero()
{
  hakans_fsr_setvalue(1);
  delay(200); // Lite extra
}

void hakans_fsr_setNextInsensitive()
{
  hakans_fsr_setvalue(2); 
}

void hakans_fsr_showValue(int pin_number)
{
  hakans_fsr_setvalue(3); 
}
// Hakan
#endif
...
void prepare_move_raw() {
  ...
  plan_buffer_line(delta[X_AXIS], delta[Y_AXIS], delta[Z_AXIS], destination[E_AXIS], (feedrate/60.0)*(feedrate_multiplier/100.0), active_extruder);
...
#ifdef HAKANS_FSR
  hakans_fsr_setzero();
  hakans_fsr_endstop_active=1;
#endif
  prepare_move_raw(); // this will also set_current_to_destination
  st_synchronize();
  endstops_hit_on_purpose(); // clear endstop hit flags
#ifdef HAKANS_FSR
  hakans_fsr_endstop_active=0;
#endif
...
inline void gcode_G29() {
  ...
  for (int yCount = 0; yCount < auto_bed_leveling_grid_points; yCount++) {
	...
	act = ProbeStay;
      ...
      setup_for_endstop_move();
#endif
      //if (verbose_level > 3) SERIAL_ECHOLNPGM("probe_pt");
...
inline void gcode_G30() {
  ...
  run_z_probe();
  SERIAL_PROTOCOLPGM("Bed X: ");
  SERIAL_PROTOCOL_F(current_position[X_AXIS] + 0.0001,3);
  SERIAL_PROTOCOLPGM(" Y: ");
  SERIAL_PROTOCOL_F(current_position[Y_AXIS] + 0.0001,3);
  SERIAL_PROTOCOLPGM(" Z: ");
  SERIAL_PROTOCOL_F(current_position[Z_AXIS] + 0.0001,3);
  SERIAL_EOL;
...
inline void gcode_M40() {
  if (code_seen('S')) {
    int pulses = code_value_short();
    hakans_fsr_setvalue(pulses);
  } else {
    hakans_fsr_setzero();
  }
}
...
#ifdef HAKANS_FSR
  SERIAL_ECHO("Not with Hakan's fsr");
#else
